  ...
  return (
    <div>
      ...
      <section className="page-banner px-5 md:px-10">
        ...
        <img className="page-banner__img" src="https://images.unsplash.com/photo-1488646953014-85cb44e25828?auto=format&fit=crop&w=1800&q=80" alt="" />
        <div className="page-banner__veil" />
        <div className="page-banner__inner reveal max-w-6xl mx-auto">
          <p className="eyebrow mb-6" style={{ color: "var(--yellow)" }}>What We Do</p>
          <h1 className="text-white" style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 800, fontSize: "clamp(40px, 6.4vw, 72px)" }}>
            Our Services
          </h1>
          <p className="mt-5 text-lg max-w-xl" style={{ color: "rgba(255,255,255,0.78)", fontFamily: "'Inter', sans-serif", fontWeight: 300 }}>
            One agency, every step of the journey, handled with care.
          </p>
        </div>
      </section>
        ...
        {serviceDetails.map((s, i) => (
          <div key={s.id} id={s.id} className="scroll-mt-28">
            ...
            <div className="flex flex-col gap-5">
              {s.paras.map((p, pi) => (
                <p key={pi} className="lead">{p}</p>
              ))}
            </div>
            <Link href={`/quote?service=${s.id}`} className="btn btn-blue mt-9">
              Get a Quote →
            </Link>
            {i < serviceDetails.length - 1 && (
              <div className="mt-16" style={{ height: "1px", background: "rgba(15,23,42,0.08)" }} />
            )}
          </div>
        ))}
      </div>
